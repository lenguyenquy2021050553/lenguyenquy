...
function createWaterDroplet(xylemPos, leafCellPos, khiKhongPos) {
    ...
    droplet.position.set(xylemPos.x, xylemPos.y, xylemPos.z - 1); // Khởi tạo phân tử nước tại vị trí mạch gỗ (xylem)
    scene5.add(droplet);

    const direction = new THREE.Vector3(0, 0, 1); // Hướng mặc định là trục Z
    let stage = 1;
    const speed = 0.05; // Tốc độ phân tử nước
    const lerpSpeed = 0.1;

    // Hàm di chuyển phân tử nước
    function animateDroplet() {
        if (stage === 1) {
            droplet.position.z += speed;  // Di chuyển phân tử nước
            ...
            droplet.position.lerp(leafCellPos, lerpSpeed);
            ...
            droplet.position.lerp(khiKhongPos, lerpSpeed);
    ...
    animateDroplet(); // Bắt đầu quá trình di chuyển
}
    ...
    function animateSugar() {
        if (stage === 1) {
            ...
            if (sugarMolecule.position.z >= leafCellPos.z + 0.5) {
                createPathCylinder(leafCellPos, sugarMolecule.position.clone(),'sugar');
                stage = 2;
            }
        } else if (stage === 2) {
            // Di chuyển tới vị trí của phloem
            sugarMolecule.position.lerp(phloemPos, lerpSpeed);
            createPathCylinder(sugarMolecule.position.clone(), phloemPos,'sugar');
...
function animateWaterDroplets(xylemPos, leafCellPos, khiKhongPos) {
    ...
    setInterval(() => {
        createWaterDroplet(xylemPos, leafCellPos, khiKhongPos);
    }, 500); // Mỗi 500ms tạo 1 phân tử nước
}
...
function animateSugarMolecules(leafCellPos, xylemPos, phloemPos) {
    ...
    setInterval(() => {
        createSugarMolecule(leafCellPos, xylemPos, phloemPos);
    }, 1000); // Mỗi 1000ms tạo 1 phân tử đường
}
...
function animate() {
    ...
    // Gọi các hàm tạo phân tử nước và đường tại một thời điểm
    animateWaterDroplets();
    animateSugarMolecules();
}
